...
var currentYear = new Date().getFullYear(); /// DOM-elements

var timelineBox = document.querySelector('.timeline__box');
var selectedDate = document.querySelector('.js-date');
var selectedDateDescription = document.querySelector('.js-sub');
var gradientHover = document.querySelector('.js-hover'); /// classes

var datesClass = 'timeline__dates';
var selectedYearClass = 'timeline__dates--active';
var showGradientClass = 'timelapse'; /// inits

generateTimeline(timelineArray); /// dinamic DOM-elements

var timelineItems = timelineBox.children;
var center = Math.round((timelineItems.length - 1) / 2);
var selectedDataIndex;
initTimeline(); /// functions
...
function arangeTimelineItemsAround(pos) {
  ...
  for (i = 0; i < timelineItems.length; i++) {
    var rotateAngle = offsetAngle * (i + position);
...
function scrollTimeline(e) {
  e = e || window.event;
  var currentDataIndex = getNodeIndex(timelineBox.querySelector('.timeline__dates--active'));
