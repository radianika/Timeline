...
//getNodeIndex(timelineBox.querySelector('.'+selectedYearClass));
...
function arangeTimelineItemsAround(pos){
	...
	for( i = 0; i < timelineItems.length; i++){
		...
		timelineItems[i].setAttribute('style', "transform : rotate(" + rotateAngle + "deg) translate(0, -22.8vw) rotate( -87deg)")
	}
}
...
if (timelineBox.addEventListener) {
  if ('onwheel' in document) {
  	timelineBox.addEventListener("wheel", function(){
		stopWindiwScroll();											 
		throttle(scrollTimeline(), 200)}
	);
  } else if ('onmousewheel' in document) {
    timelineBox.addEventListener("mousewheel", throttle(scrollTimeline, 200));
	} else {
    timelineBox.addEventListener("MozMousePixelScroll", throttle(scrollTimeline, 200));
  }
} else { 
  timelineBox.attachEvent("onmousewheel", throttle(scrollTimeline, 200));
}
